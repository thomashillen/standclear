...
// MTA's subway alerts feed. Same host as the train feeds, separate endpoint.
// Payload is GTFS-RT protobuf with alert entities instead of vehicle/trip
// updates. No API key required since late 2023.
const ALERTS_URL =
...
// The protobuf decoder returns `effect` as a numeric enum, not a name.
// Map it back to the canonical GTFS-RT names for display and logic.
// Values per https://gtfs.org/realtime/reference/#message-alert.
...
// MTA's feed mostly publishes effect=UNKNOWN_EFFECT and packs the real
// state into the header. When the enum is informative, trust it; otherwise
// scan the header for common disruption phrasing. Order matters — check
// strongest indicators first so "No [4] service" beats "partially running".
function severityOf(effect: string, header: string): AlertSeverity {
...
export async function GET() {
  let body: AlertsResponse = { generatedAt: Date.now(), alerts: [] };
  ...
  try {
    ...
    for (const entity of feed.entity) {
      ...
      // Filter to alerts active right now. If no active_period is given the
      // alert is considered always-on; otherwise at least one window must
      // contain the current time. Planned-future alerts are still rendered
      // on the full list but we scope this API to "what matters now".
  ...
  return NextResponse.json(body, {
    // Cache briefly at the edge — alerts change far less often than trains
    // and the client polls at a low cadence anyway.
