...
const ENTRIES: Entry[] = [
  {
    ...
    title: "MVP build",
    changes: [
      { type: "fixed", text: "Choosing a destination from Search now requests the rider's location and automatically fills Current location as the trip origin. Previously Search only reused a location obtained from Near Me, so first-time directions silently left From empty without showing the browser permission prompt." },
      ...
      { type: "fixed", text: "The Home/Work commute card now prefers a route with a reachable live departure over a static route whose first leg currently has no matching trains. A direct 4 train route can still appear under 'See all routes,' but it no longer wins the one-card recommendation just because the planner filled in a fallback wait." },
      ...
      { type: "changed", text: "The grabber at the top of every mobile sheet now has a full 44px centered touch target, making expand and collapse taps easier to catch one-handed on a moving train. The visible pill and sheet layout stay exactly where they were, and the wider hit area remains clear of the header's edge controls." },
      { type: "changed", text: "Screen readers now hear the same sentence for a stale-position warning everywhere it appears: the station arrival row, line panel's per-direction ETA chip, and trip planner used to re-create the phrasing and rounding independently, so any could drift from the visible 'Updated Nm ago' badge. The spoken age is now single-sourced alongside the visible label so the two can never disagree. No change for sighted riders." },
